...
class Agent(object):
    ...
    def value_estimates(self, contexts=None, cache_index=None,return_std=False,arm=None,batch=True):
        ...
        if cache_index is not None:
            assert self.is_prepared(), "When an int is given as context the expert should be prepared in advance"
            self.mu, self.sigma = np.copy(self.cached_predictions[cache_index])
        else:
            self.mu, self.sigma = self.predict_normalized(contexts,arm=arm,batch=batch)
        if return_std:
    ...
    def update_confidence(self, contexts=None, value_mode=False,confidence_rho=None,oracle_confidence=True):
        ...
        pred_val, uncertainty = self.value_estimates(contexts, return_std=True,batch=False)
        assert len(np.shape(pred_val))==len(np.shape(contexts)),(np.shape(pred_val),np.shape(contexts))
    ...
    def cache_predictions(self, bandit, trials):
        ...
        if not self.is_prepared(bandit.cache_id) or len(self.cached_predictions) < trials:
            ...
            assert np.shape(self.cached_predictions)==(2,trials,bandit.k),np.shape(self.cached_predictions)
    ...
    def cache_confidences(self, bandit,  value_mode, confidence_rho,oracle_confidence=True):
        ...
        self.cached_votes[np.arange(len(self.cached_votes)), np.argmax(self.cached_probabilities, axis=1)] = 1


        self.total_expected_reward = np.sum(np.sum(self.cached_votes * bandit.cached_rewards,axis=1))
        if oracle_confidence:
            
            self.cached_confidences = np.sum(self.cached_votes * bandit.cached_rewards, axis=1)[:, np.newaxis]
            ...
            assert (0 <= self.cached_confidences).all() and (self.cached_confidences <= 1).all()
            
            self.cached_confidences = self.cached_confidences * (1 - RANDOM_CONFIDENCE) + RANDOM_CONFIDENCE
            if not value_mode:
                self.cached_confidences[:, :] = gmean(np.clip(self.cached_confidences, 1e-6, 1), axis=1)[:, np.newaxis]
...
class KernelUCB(Agent):
    ...
    def prior_play(self, steps=0, bandit=None, spread=None):
        ...
        assert 0 < spread < 1 + 2, "Hypercube sides should be strictly positive but are {}".format(spread)
    ...
    def update_kinv(self,arm):
        ...
        self.k_inv_history[arm] = np.linalg.inv(self.kernel(contexts) + np.identity(len(contexts)) * self.gamma)
    ...
    def predict_normalized(self, contexts,arm=None, slice_size=None,batch=True):
        ...
        if not batch:
            ...
            if np.shape(contexts) in ((1,self.bandit.dims),(self.bandit.dims,)):
                values = np.array([self.predict_normalized(contexts.reshape((1,-1)),arm=a,batch=True)[0] for a in range(self.bandit.k)])
            elif np.shape(contexts)==(self.bandit.k,self.bandit.dims):
                values = np.array([self.predict_normalized(contexts[a],arm=a,batch=True) for a in range(self.bandit.k)])

            return values[:,0],values[:,1]

        if arm is None:
            values = np.array([self.predict_normalized(contexts,arm=a) for a in range(self.bandit.k)])
    ...
    def ucb_values(self, contexts=None):
        ...
        mu,sigma = np.array([self.value_estimates(contexts,return_std=True,arm=a) for a in range(self.bandit.k)]).T[0]
